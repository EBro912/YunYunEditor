<script lang="ts">
  ...
  function nudge(field: 'Tick' | 'Lane' | 'Duration', delta: number) {
    ...
    mutateActiveLevel((lvl) => {
      // Rush notes occupy Lane and Lane+1, so the left lane must stay within [min, max-1] —
      // clamping to LANE_RANGE.max would push the right half off the visible playfield and
      // the renderer would silently skip the note.
      const map = (laneMax: number) => (n: AnyNote): AnyNote => {
        if (!n.id || !ids.has(n.id)) return n;
        const nn: AnyNote = { ...n };
        if (field === 'Tick') nn.Tick = Math.max(0, n.Tick + delta);
        else if (field === 'Lane') nn.Lane = Math.max(LANE_RANGE.min, Math.min(laneMax, n.Lane + delta));
        else if (field === 'Duration' && 'Duration' in nn) (nn as HoldNote).Duration = Math.max(1, (n as HoldNote).Duration + delta);
        return nn;
      };
      return {
        ...lvl,
        SingleNotes: lvl.SingleNotes.map(map(LANE_RANGE.max)) as SingleNote[],
        HoldNotes: lvl.HoldNotes.map(map(LANE_RANGE.max)) as HoldNote[],
        RushNotes: lvl.RushNotes.map(map(LANE_RANGE.max - 1)) as RushNote[],
      };
    });
  }
</script>
